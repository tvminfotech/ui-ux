...
export class WorkSpacePrototypeService {
  ...
  onSaveLink(value: any) {
    this.ngxService.start();
    return this.http.post<any>(`${environment.apiUrl}poc/doc/details`, value, this.header)
      .pipe(map(data => {
        this.ngxService.stop();
        return data;
      }),
  ...
  getLinkDetails(id: any) {
    this.ngxService.start();
    return this.http.get<any>(`${environment.apiUrl}poc/doc/details?micrositeId=${id.micrositeId}&pocId=${id.pocId}&wsBoardId=${id.wsBoardId}&docName=Prototype`, this.header)
    .pipe(map(data => {
      this.ngxService.stop();
      return data;
    }),
